    ...
    for batch_idx, raw_batch in enumerate(eval_data_loader):
        ...
        for _v in range(len(dec_seq_list)):
            ...
            for example_idx, (cur_gen_sen, cur_meta) in enumerate(zip(dec_seq, meta)):
                ...
                with open(os.path.join("your_path/VAR-main/captions/activitynet", "val{}".format(opt.json_path)), 'r') as file:
                    data = json.load(file)
                    for item in data:
                        name = list(item.keys())[0]
                        if name == cur_meta['name']:
                            dict = item[name]
                            if 'sentences' in dict and dict['sentences']:
                                result_sentence = dict['sentences']
                cur_data = {
                    "sentence": eval_data_loader.dataset.convert_ids_to_sentence(
                        cur_gen_sen.cpu().tolist()),
                    "gt_sentence": result_sentence
                }
...
def translate_w_metrics(checkpoint, eval_data_loader, opt, model=None, eval_mode='val'):
