...
class RelationHead(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.decoder = nn.Linear(config.hidden_size * 2, 2)
        self.attention_vf = Attention_via_features(768)
        self.attention_lo = Attention_local(768)

    def forward(self, unmasked_feature, masked_feature_list, complete_flag=False):
        output_list = []
        if not complete_flag:
            for masked_feature in masked_feature_list:
                att_feature_vf = self.attention_vf(masked_feature, unmasked_feature)
                att_feature_lo = self.attention_lo(masked_feature)
                output_logits = self.decoder(torch.cat((att_feature_vf, att_feature_lo), dim=2)).view(-1, 2)[:-self.config.max_v_len, :]
                output_list.append(output_logits)
        else:
            # complete graph while validation
            for i, masked_feature in enumerate(masked_feature_list):
                att_feature_vf = self.attention_vf(masked_feature, unmasked_feature[i])
                att_feature_lo = self.attention_lo(masked_feature)
                output_logits = self.decoder(torch.cat((att_feature_vf, att_feature_lo), dim=2)).view(-1, 2)[:-self.config.max_v_len, :]
                output_list.append(output_logits)
...
class Compensate_Head(nn.Module):
    def __init__(self, config):
        super(Compensate_Head, self).__init__()
        self.decoder = nn.Linear(config.hidden_size * 2, 2)
        self.attention_vf = Attention_via_features(768)
        self.attention_lo = Attention_local(768)

    def forward(self, current_feature_list, next_feature_list):
        output_list = []
        for current_feature, next_feature in zip(current_feature_list, next_feature_list):
            att_feature_vf = self.attention_vf(current_feature, next_feature)
            att_feature_lo = self.attention_lo(current_feature)
            output_logits = self.decoder(torch.cat((att_feature_vf, att_feature_lo), dim=2)).view(-1, 2)
            output_list.append(output_logits)
        return output_list
