...
from videollava.utils import order_pick_k, disable_torch_init
...
class ModelArguments:
    model_name_or_path: Optional[str] = (
        field(
            default="cache_dir/models--LanguageBind--Video-LLaVA-7B/snapshots/e16778e47e589512d7e69f964850c8cad775a335"))
    version: Optional[str] = field(default="llava_sg_video")
    ...
    # ===================================================================
    image_tower: Optional[str] = field(default="LanguageBind/LanguageBind_Image")
    video_tower: Optional[str] = field(default="LanguageBind/LanguageBind_Video_merge")
    # ===================================================================
...
class DataArguments:
    ...
    data_path: Optional[str] = field(
        default="datasets/ActivityNet_QA/annotations/train_sg_grounding_DINO_filtered.json")
    image_folder: Optional[str] = field(default="llava_all_image_video")
    video_folder: Optional[str] = field(default="datasets/ActivityNet_QA/train")
    num_frames: int = 8
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    output_dir: str = "./checkpoints/videollava-7b"
    ...
    # ================================================
    tokenizer_model_max_length: Optional[int] = 3072
    # ================================================
...
def preprocess_sg_video(
    ...
    for i, source in enumerate(sources):
        conv.messages = []
        video_tokens = 'video:<image><image><image><image><image><image><image><image>\n'

        if use_graph:
            video_sg_tokens = 'video graph: '
            for id, ground_res in enumerate(source['grounding']):
                triplet_lst = ground_res['triplet']

                scene_graph_tokens = 'frame-{}:'.format(id + 1)
                if triplet_lst:
                    for triplet in triplet_lst:
                        s_name, p_name, o_name = triplet
                        triplet_token = '<entity>' + p_name + '<entity> '
                        scene_graph_tokens += triplet_token
                else:
                    scene_graph_tokens += 'None '

                video_sg_tokens += scene_graph_tokens

            video_sg_tokens += '\n'
            question = video_tokens + video_sg_tokens + source['question']
        else:
            question = video_tokens + source['question']
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version.startswith("sg_video"):
        return preprocess_sg_video(sources, tokenizer, use_graph=use_graph)
...
class StructuredVideoDataset(Dataset):
    ...
    def __init__(self, data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
        super(StructuredVideoDataset, self).__init__()
        # ================================================
        list_data_dict = []
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            cur_len += len(sample['answer'].split())
            # calculate scene length
            sg_len = 0
            for frame_id in range(len(sample['grounding'])):
                for i in sample['grounding'][frame_id]['score']:
                    if i:
                        sg_len += 1
            cur_len += sg_len + sg_len // 2
            length_list.append(cur_len)
        return length_list
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            video_name = self.list_data_dict[i]['question_id']
            video_file = video_name[: video_name.rfind('_')]
            video_folder = self.data_args.video_folder
            video_processor = self.data_args.video_processor
            video_file = video_file if isinstance(video_file, list) else [video_file]
            video_file = order_pick_k(video_file, MAX_VIDEO_LENGTH)
            # video = [os.path.join(video_folder, file) for file in video_file]
            video_path = os.path.join(video_folder, video_file[0])
            for fmt in VIDEO_FORMATS:
                if os.path.exists(video_path + fmt):
                    video_path = video_path + fmt
                    break
            video = [video_path]

            image = [video_processor(i, return_tensors='pt')['pixel_values'][0] for i in video]  # fake image

            entity_dir = os.path.join('datasets/ActivityNet_QA/embeddings_filtered', sources[0]['question_id'])

            embed_lst = []
            for i in range(8):
                entity_path = os.path.join(entity_dir, "{}.pt".format(i))
                embed = torch.load(entity_path)
                embed_lst.append(embed)

            # image = [torch.randn(3, 8, 224, 224) for i in video]  # fake image
            # sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
            # print('after preprocess_multimodal', sources[0])
            data_dict = preprocess(sources, self.tokenizer, has_image=True, use_graph=False)
            # print('after preprocess', data_dict['input_ids'])
            # ==========================================================================================================
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in instances[0]:
            ...
            entities = [instance['entity'] for instance in instances]
...
def make_scene_graph_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                 data_args) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = StructuredVideoDataset(tokenizer=tokenizer,
                                           data_path=data_args.data_path,
                                           data_args=data_args)
    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer)
    return dict(train_dataset=train_dataset,
...
def train():
    ...
    if data_args.scene_graph:
        # with scene graph
        data_module = make_scene_graph_data_module(tokenizer=tokenizer,
                                                   data_args=data_args)
    else:
        # without scene graph
