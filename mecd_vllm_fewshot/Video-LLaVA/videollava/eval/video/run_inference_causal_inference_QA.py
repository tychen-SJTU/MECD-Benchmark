...
def parse_args():
    ...
    return parser.parse_args([  '--model_path', 'your_path/hg_models/Video-LLaVA-7B',
                                '--cache_dir', 'cache_dir/',
                                '--video_dir', 'your_path/Video-LLaVA/eval/GPT_Multi_Videos_QA/Activitynet_Multi_Videos_QA/val',
                                '--gt_file_qa', 'your_path/MECD/VAR-main/causal_qa_val.json',
...
def run_inference(args):
    ...
    for qasample in tqdm(gt_questions_answers):
        ...
        # one_clips_str = ','.join(map(str, one_input_list))
        # one_input_str = f"All Clip {one_clips_str} with one image as input. "
        # if len(times) < 8:
        #     two_clips_str = ','.join(map(str, two_input_list))
        #     two_input_str = f"While Clip {two_clips_str} with two images as input. "
        ...
        # relation_prompt.append(no_causal_prompt)
        relation_prompt.append(causal_prompt)
        # relation_prompt.append(one_input_str)
        # if len(times) < 8:
        #     relation_prompt.append(two_input_str)
        relation_prompt = ''.join(relation_prompt)
        times.append(duration)

        question = relation_prompt + question
    ...
    with open('your_path/Video-LLaVA/causal_qa_video_llava_v1.json', 'w') as f:
