...
def parse_args():
    ...
    return parser.parse_args([  '--model_path', 'your_path/Video-LLaVA/checkpoints/videollava-7b',
                                '--cache_dir', 'cache_dir/',
                                '--video_dir', 'your_path/Video-LLaVA/eval/GPT_Multi_Videos_QA/Activitynet_Multi_Videos_QA/val',
                                '--gt_file_qa', 'your_path/Video-LLaVA/eval/GPT_Multi_Videos_QA/Activitynet_Multi_Videos_QA/QA_val_reason_fewshot.json',
                                '--output_dir', 'your_path/Video-LLaVA/eval/GPT_Multi_Videos_QA/Activitynet_Multi_Videos_QA/Video-LLaVA-7B',
...
def get_model_output(model, video_processor, tokenizer, video, qs, task_id, args):
    ...
    video_tensor = video_processor.preprocess(video, return_tensors='pt')['pixel_values'].bfloat16().to(args.device)
...
def run_inference(args):
    ...
    causal_inference_file = 'your_path/Ask-Anything/video_miniGPT4/val_caption_small3_updated_large.json'
    ...
    for task_id, sample in tqdm(enumerate(causal_inference_list)):
        ...
        main_video_path = 'your_path/Video-LLaVA/eval/GPT_Multi_Videos_QA/Activitynet_Multi_Videos_QA/val/v__1vYKA7mNLI.mp4'
        # main_video_path = get_video_format(main_video_path)
        # multi_video_set = multi_video_set[:args.support_num]
        # try:
        # Run inference on the video and add the output to the list
        
        output = get_model_output(model, processor['video'], tokenizer, main_video_path, question, task_id, args)
    ...
    with open('your_path/Video-LLaVA/videollava_relation_pred.json', 'w') as f:
