...
def load_video(video_path, timestamps, duration_annotation, num_segments=8, return_msg=False, resolution=224):
    ...
    frame_indices = np.array([min(int(time * num_frames / duration_annotation), num_frames-1) for time in frame_stamps])
...
for task_id, sample in tqdm(enumerate(causal_inference_list)):
    ...
    if len(description) > 3:
        ...
        causal_prompt = task + description + task1 + causal_prompt
        ...
        print(output)
        video_dict = {"question": question, "answer": gt, "pred": output}
        all_list.append(video_dict)

with open('your_path/video_chat2/videochat2_causal_ep_vicuna.json', 'w') as f:
