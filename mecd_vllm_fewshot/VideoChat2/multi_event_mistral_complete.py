...
def answer(conv, model, img_list, do_sample=True, max_new_tokens=200, num_beams=1, min_length=1, top_p=0.9,
    ...
    with torch.no_grad():
        ...
        if '<s>' in output_text:
            output_text = output_text.split('<s>')[1]  # remove the stop sign </s>
...
if __name__ == '__main__':
    ...
    for task_id, sample in tqdm(enumerate(causal_inference_list)):
        ...
        question_tail = ("Your probability output lists group(lengths = [{}], ".format(','.join(map(str, lengths))) +
        ...
        gt_answer = ce_sample['all_relation']
