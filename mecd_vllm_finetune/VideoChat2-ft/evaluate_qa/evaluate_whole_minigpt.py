...
results_path = '/home/hbliu/Ask-Anything/video_chat2/results/whole_test/minigpt4video_gemini.json'
...
for video, preds in results.items():
    for q_id, pred in preds.items():
        total += 1
        #breakpoint()
        pred_r = find_first_digit(pred['pred_org'])
        pred_r_s = find_first_digit(pred['pred_r'])
