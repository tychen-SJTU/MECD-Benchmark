...
state_dict = torch.load("/home/hbliu/videochat_models/videochat2_7b_stage3.pth", "cpu")
...
if __name__ == '__main__':
    results = {}
    test_csv_file = '/home/hbliu/Video-LLaVA/test_order.csv'
    ...
    for index, row in df.iterrows():
        ...
        answer_id = [a0,a1,a2,a3,a4].index(gt_answer)
        # answer_r_id = option_id_dict[[r_a0,r_a1,r_a2,r_a3,r_a4].index(gt_answer)]
        if video_id not in finished_video_ids:
            # Load new video
            print('Test video: ', video_id)
            finished_video_ids.append(video_id)
            vid_path = '/home/hbliu/videos/NExTVideo/' + str(video_id) + '.mp4'
        ...
        prompt = task + question_a + question_prompt
        chat.messages.append([chat.roles[0], f"<Video><VideoHere></Video>\n"])
        ask(prompt, chat)
        llm_message = answer(conv=chat, model=model, do_sample=False, img_list=img_list, max_new_tokens=100, answer_prompt=answer_prompt, print_res=True)[0]
        final_output = return_prompt + llm_message.strip().split('\n')[0]

        # record results
        if video_id not in results:
            results[video_id] = {}
        results[video_id][qid] = {'question':question, 'gt_answer':gt_answer, \
                                  'gt_idx': answer_id, 'pred':final_output}
        if index % 500 == 0:
            with open('/home/hbliu/Ask-Anything/video_chat2/results/videochat_results_all_test_no_r.json', 'w') as f:
                json.dump(results, f)
