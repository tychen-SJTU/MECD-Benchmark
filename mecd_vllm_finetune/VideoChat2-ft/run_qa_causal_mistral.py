...
# Your path of videochat2_stage3 checkpoint
state_dict = torch.load("./VideoChat2_stage3_Mistral_7B/videochat2_mistral_7b_stage3.pth", "cpu")
...
if __name__ == '__main__':
    ...
    qa_file = './question_answering/QA.json'
    ...
    # Iterate over each sample in the ground truth causal graph annotation file
    # You can alternate with the causal graph output by each model.
    # Current timestamps are sourced form ground-truth annotation, you can alternate it with any 
    # dense video captioning model as well.
    '''
    all_list = []
    global sample, key
    for qasample in tqdm(qa_list):
        ...
        task_Prompt = ("You can give an answer by carefully thinking step by step by analyzing the visual contents"
                       " (and causal relations when facing complex question).")

        # Your path to video dataset
        video_name = '.../dataset/Activitynet/v1-3/train/' + key
        video_path = get_video_format(video_name)
        if video_path is None:
            video_name = '.../Activitynet/v1-3/val/' + key
            video_path = get_video_format(video_name)
    ...
    with open('./video_chat2/videochat2_causal_qa_mistral.json', 'w') as f:
