...
state_dict = torch.load("./VideoChat2_FT_Checkpoints/ckpt_latest.pth", "cpu")
...
if __name__ == '__main__':
    ...
    for task_id, sample in tqdm(enumerate(causal_inference_list)):
        ...
        sample_set = {'id': task_id, 'question': question}
        
        vid_val_root = '/mnt/sdb/dataset/eventbench/supple_mecd'
        ...
        while True:
            if output.endswith('0') or output.endswith('1'):
                break
            else:
                output = output[:-1]
    ...
    #         relation = {"name": key, "pred": output.tolist()}
