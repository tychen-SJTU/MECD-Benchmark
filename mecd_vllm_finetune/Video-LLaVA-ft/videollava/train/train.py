...
class ModelArguments:
    model_name_or_path: Optional[str] = field(
        default="your_path/Video-LLaVA/cache_dir/models--LanguageBind--Video-LLaVA-7B/snapshots/e16778e47e589512d7e69f964850c8cad775a335")
...
class TrainingArguments(transformers.TrainingArguments):
    cache_dir: Optional[str] = field(default="your_path/Video-LLaVA/cache_dir")
    ...
    output_dir: str = "./checkpoints/videollava-7b-lora"
...
def preprocess_causality(
    ...
    # for i, source in enumerate(sources):
        ...
        # print(source.dtype)
        # print(len(source))
    key = [s for s in sources][0]
    ce_sample = sources[key]
    ...
    question = video_tokens + question_head + question_body + question_tail
    answer = ce_sample['relation'][0]
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version.startswith("llava_causal_inference"):
        return preprocess_causality(sources, tokenizer)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
...
class MECDDataset(Dataset):
    def __init__(self, causal_inference_list, tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
    ...
    def modality_lengths(self):
        length_list = []
        for sample in self.causal_inference_list:
            key = [s for s in sample][0]
            ce_sample = sample[key]
            cur_len = len(ce_sample['sentences'])
            length_list.append(cur_len)
        return length_list

    def __getitem__(self, task_id) -> Dict[str, torch.Tensor]:
        sample = self.causal_inference_list[task_id]
        key = [s for s in sample][0]
        # ce_sample = sample[key]
        # print(conversation_lib.default_conversation.version)
        conversation_lib.default_conversation.version = "llava_causal_inference"
...
def train():
    ...
    if model_args.version == "v0":
        ...
        if model_args.version in conversation_lib.conv_templates:
            conversation_lib.default_conversation = conversation_lib.conv_templates["llava_causal_inference"]
        else:
            conversation_lib.default_conversation = conversation_lib.conv_templates["llava_causal_inference"]
    ...
    data_module = make_mecd_data_module(tokenizer=tokenizer,
                                        data_args=data_args)
